...
function fRaidItem.Add(arg)
	...
	if not id and type(arg) == 'string' then
		...
		error('Invalid arg.  fRaidItem.Add(arg).  arg should be item id (number) or item link(string)')
	end
...
function fRaidItem.Scan()
	for i = 1, GetNumLootItems() do
		if LootSlotIsItem(i) then
			fRaid:Debug('Found slot ' .. i .. ' ' .. GetLootSlotLink(i))
			fRaidItem.Add(GetLootSlotLink(i))
		end
	end
end

function fRaidItem.GetInfo(id)
	local items = fRaid.db.global.ItemList
	for idx,info in ipairs(items) do
		if info.id == id then
			return info
